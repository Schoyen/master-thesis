...
tdcisd = TDCISD(odho, integrator=integrator, verbose=True)
tdcisd.compute_ground_state()
tdcisd.set_initial_conditions()

rho_tdcisd = tdcisd.compute_particle_density()
write_data(
    os.path.join(path, "rho_tdcisd_real.dat"), odho.grid, rho_tdcisd.real
)
...
overlap[0] = tdcisd.compute_time_dependent_overlap()

for i, amp in tqdm.tqdm(
    enumerate(tdcisd.solve(time_points)), total=num_timesteps - 1
):
    overlap[i + 1] = tdcisd.compute_time_dependent_overlap()

write_data(
    os.path.join(path, "overlap_tdcisd_real.dat"),
