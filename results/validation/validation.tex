\chapter{Validation}
    ...
    \section{The one-dimensional harmonic oscillator}
        An excellent starting point for validaion is the one-dimensional
        ...
        one-body particle densities using the general Hartree-Fock method and
        CISD, i.e., full configuration interaction for two particles.
        In the lower plot in \autoref{fig:one-body-density-zanghellini} we
        demonstrate that CCSD reproduces the exact solution for the one-body
        particle density as achieved from configuration interaction to a high
        precision by computing the absolute difference between each point on the
        grid, that is,
        \begin{align}
    ...
    \section{Two-electron molecule}
        ...
        The basis is run through our general Hartree-Fock solver which finds the
        Hartree-Fock molecular orbitals and changes to this basis.
        ...
        \SI{0.1}{\hartree/\hslash}$ which corresponds to a wave length of
        $\lambda = \SI{456}{\nano\meter}$.
        This linear envelope is slightly unconventional as compared to the more
    ...
    \section{Summing up the validation}
        ...
        system and the interface towards PySCF are working.
        We've also demonstrated that we support several dipole laser fields
        and that we can compute various quantities from all methods.
