\chapter{Quantum mechanics}
    ...
    \section{Laser fields}
        ...
        \subsection{Measuring energy transitions}
            ...
            For $t > T$ we have $\hamil(t > T) = \hamil(0)$, and the
            time-evolution of an arbitrary state $\ket{\psi(t)}$ can be
            described by
            \begin{align}
                \ket{\psi(t > T)}
                = \sum_{k = 0}^{\infty} c_k(t)\ket{\phi_k}
                = \sum_{k = 0}^{\infty}
                c_k(0) e^{-i\energy_k t/\hslash}\ket{\phi_k},
            \end{align}
            where the time-evolution is kept in the coefficients and
            $\ket{\phi_k}$ are the same eigenstates found in the
            ...
            \subsubsection{Autocorrelation}
                ...
                $\ket{\psi}$ for $t > T$ \cite{robinett20041,
                pedersen2018symplectic},
                \begin{align}
                    A(t)
                    &= \braket{\psi}{\psi(t)}
                    =
                    ...
                    \sum_{i = 0}^{\infty}
                    \sum_{j = 0}^{\infty}
                    c^{*}_i(0) c_j(0) e^{i E_i t/\hslash}
                    \delta_{ij}
                    =
                    \sum_{i = 0}^{\infty}
                    \abs{c_i(0)}^2 e^{i E_i t/\hslash},
                \end{align}
                where we've used that the eigenstates are orthonormal.
                In the case of the initial state being the ground state the
                autocorrelation takes on the form
                \begin{align}
                    A(t)
                    &= \braket{\phi_0}{\psi(t)}
                    = \sum_{i = 0}^{\infty}
            ...
            \subsubsection{Dipole moment}
                Computing the expectation value of the dipole moment for $t > T$
                we have
                \begin{align}
                    \expval{\vfg{d}(t)}
                ...
                time-independent system and the right-hand side of the
                expectation value of the dipole moment is not something we can
                evaluate directly.
                However, by inspection we see that the right-hand side bears
                resemblance to the \emph{discrete Fourier transform}, or perhaps
                more specificaly, the inverse of the discrete Fourier
                transform.\footnote{%
                    Note that whether or not it is the transform or the inverse
                    is not so important as the transform can be made into the
                    inverse and vice versa.
                    It is just that the inverse Fourier transform by convention
                    has a positive sign in the exponential.
                }
