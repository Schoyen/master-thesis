\section{One-dimensional quantum dots}
    ...
    \subsection{Discretizing the problem}
        ...
        $\vfg{h}$ where $\vfg{\psi}$ becomes a matrix with each column
        representing an eigenstate, and the rows the evaluation of the
        eigenstates on the grid.
        Some care must be taken when selecting which eigenpairs to include in
        the further analysis.
        The quality of the eigenpairs is dependent on the number of grid points
        $n$ and the lower lying eigenpairs, i.e., lowest excitations, will in
        general be approximated better than the higher lying states.
        % TODO: Include this analysis in the results section.
        % TODO: Verify that lower lying states are better than higher states.
        ...
        set the $\psi_p(x_0) = \psi_p(x_{n + 1}) = 0$.
