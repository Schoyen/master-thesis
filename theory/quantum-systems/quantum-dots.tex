...
\section{Two-dimensional quantum dot}
    ...
    \subsection{Two-dimensional double well quantum dot}
        ...
        \end{align}
        We note that for odd values of $\Delta m_{pq}$ we will get sines from
        Euler's formula, but they will have opposite signs due to the latter
        term being shifted half a revolution ahead of the former term.
        This means that $\tilde{\mathcal{C}}_{pq} = 0$ when $\Delta m_{pq}$ is
        odd.
        Even values of $\Delta m_{pq}$ gives back the cosines from Euler's
        formula.
