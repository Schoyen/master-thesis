\chapter{Configuration interaction}
    ...
    \section{Time-independent configuration interaction}
        ...
        \subsection{Size-consistency and size-extensivity}
            ...
            \end{align}
            where $A$ and $B$ are two non-interacting systems, and $AB$ are the
            combined non-interacting systems.
            This tells us that the sum of the energy of the two consituent parts
            should equal the total energy of the combined system.
            A generalization of this concept is size-extensivity
            \cite{shavitt2009many} which extends the size-consistency
            requirement to $N$ systems.
            If we consider two non-interacting systems $A$ and $B$ with two
            particles each, the full configuration-interaction method for each
            system separately would be at the singles-and-doubles truncation.
            However, the combined system consists of four particles and the
            singles-and-doubles truncation would be unable to include all
            excitations.
            This means that we would violate \autoref{eq:size-consistency}
            unless the left-hand side includes the triple and quadruple
            excitations as well.
            The fact that truncated configuration interaction is not
            size-consistent motivates the need for better approximations to the
