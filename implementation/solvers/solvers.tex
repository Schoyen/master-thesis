...
energy = np.trace(np.dot(term, density_matrix))
    ...
    \section{Coupled cluster}
        ...
        \subsection{Non-orthogonal ground state solver}
            ...
            orbitals rotations by \cite{ugur-occ}
            \begin{gather}
                \Delta (\noccten^{u})^{(i)}
                = -\frac{1}{D^a_i}
                \left.
        ...
        \subsection{Time-evolution with static orbitals}
            ...
            As the integration schemes assume a vector of derivatives, we ravel
            the amplitude tensors to represent them as a one-dimensional array
